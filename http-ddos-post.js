...
const { HTTP2_HEADER_CONTENT_TYPE, HTTP2_HEADER_CONTENT_LENGTH } = http2.constants;
...
postdata = generateRandomString(15,20)
Leng = Buffer.byteLength(postdata, 'utf8')
   hd = {}
     header = {
    ':method': 'POST'
...
header['cache-control']= Math.random() < 0.5 ? 'no-cache, no-store, private' : `max-age=${randstra(5)}`;
header[':scheme']= 'https';
header[HTTP2_HEADER_CONTENT_TYPE]= 'application/x-www-form-urlencoded';
header[HTTP2_HEADER_CONTENT_LENGTH]= Leng.toString();
